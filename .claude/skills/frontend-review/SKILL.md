...
- 子コンポーネント・子hookに新規で状態取得ロジック（useMediaQuery等の外部状態購読）を追加する場合、呼び出し階層の上位（親コンポーネント・親hook）が既に同じ値を保持していないか確認する（保持している場合はpropsで受け渡す）
...
- コンポーネントファイル内で Hooks（`useState`/`useEffect`/`useMediaQuery` 等）を呼び出す箇所が、Rules of Hooks に違反していないか
  - `&&` / `||` の右辺で Hooks を呼んでいないか（短絡評価で呼び出しがスキップされ、レンダリングごとに Hooks の呼び出し回数が変わる）
  - `if` 文の中・早期 return の後・ループの中で Hooks を呼んでいないか
  - 違反がある場合、両方の Hooks を先に呼んでから結果同士を変数で組み合わせる形に修正する
...
- `use` で始まるファイル（`use*.ts`, `use*.tsx`）または `hooks/` ディレクトリ配下のファイルが変更に含まれる場合、hooks-reviewer エージェントを呼び出して結果を取得する
  ...
  - hooks ファイルの変更がない場合でも、コンポーネントファイル内に Rules of Hooks 違反がないかは上記「コーディング規約」のチェックで必ず確認する（hooks-reviewer への委譲有無に関わらず省略しない）
