...
type propsType = {
    videoComment: VideoCommentThreadItemType,
}

export function CommentContent(props: propsType) {

    console.log("CommentContent render");

    const videoComment = props.videoComment;
    // コメントスレッドの詳細情報
    const snippet = videoComment.snippet;
    // 最上位コメント（親コメント）の詳細情報
    const parentComment = snippet.topLevelComment;
    const parentCommentSnippet = parentComment.snippet;
    // コメント本文
    const parentCommentText = parentCommentSnippet.textDisplay;
    // 投稿日
    ...
    // 返信コメントのリスト
    const replys = videoComment.replies;
    // 返信コメントの詳細情報
