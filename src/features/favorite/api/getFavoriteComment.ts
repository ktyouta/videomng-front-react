...
export function getFavoriteComment(props: PropsType) {

    const endpoint = props.videoId ? `${VIDEO_MNG_PATH}${ENV.FAVORITE_COMMENT}`.replace(`:videoId`, props.videoId) : ``;

    return useQuery({
        queryKey: favoriteVideoKeys.favoriteComment(props.videoId),
        queryFn: async () => {
